...
# Take note not to try this again
				...
				self.alignedOut =map(int,self.alignedOut)	
				self.alignedIn = np.append(self.alignedIn, self.Used[findAligned])
				self.alignedIn =map(int,self.alignedIn)
# Alignment checks for deletion
		if (self.selectedAction == self.actionDelete):
			findAligned = np.asarray(np.where(self.alignedIn == nu)[0],int)
			numAligned = findAligned.size
			if (numAligned > 0):
			#	print 'zz',findAligned[:]
			...
			#	self.alignedOut = np.delete(self.alignedOut, findAligned, 0)
			    ...
			    reinstated=[]
			    if (self.verbose):	
				    print findAligned,len(self.alignedIn),len(self.alignedOut)
			    for idx in findAligned[::-1]:
				if (self.verbose):
					print idx
				reinstated.append(self.alignedOut[idx])
				self.alignedIn = np.delete(self.alignedIn, idx, 0)
				self.alignedOut = np.delete(self.alignedOut, idx, 0)
#*****************
# Action phase
#*****************
...
# Full re-computation of statistics after beta update
		...
		if (self.verbose):
			print "{0:4d} - L={1:10.7f} - Gamma={2:10.7f} (M={3:4d}) - s={4:8.6f}".format(self.loop,self.logML[0]/N, np.sum(self.Gamm), M, np.sqrt(1.0/self.beta))
