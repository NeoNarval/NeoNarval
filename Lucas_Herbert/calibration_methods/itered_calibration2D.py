...
import calibration_methods.interpolated_conversion2D as cnv2D
...
def compute_order_first_conversion(order,max_detection,precision,converted_lambdas_file_path):
    ...
    conversion_results = cnv2D.polynomial_interpolation_legendre2D(spikes_lambdas,spikes_indices,order)
    lambdas_Angstrom = conversion_results
    ...
    order_lambdas_indices = [i for i in range(order_len)]
    plt.figure(18)
